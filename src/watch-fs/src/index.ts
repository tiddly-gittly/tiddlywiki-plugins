...
import { InverseFilesIndex } from './inverseFileIndex';
...
export interface ITWWiki {
  watchFs: {
    inverseFilesIndex: InverseFilesIndex;
  };
}
...
function FileSystemMonitor() {
  ...
  $tw.wiki.watchFs = { inverseFilesIndex: new InverseFilesIndex() };
  // folder to watch
  // non-tiddler files that needs to be ignored

  if (typeof $tw === 'undefined' || !$tw?.node) return;

  const watchPathBase: string = path.resolve($tw.boot.wikiInfo?.config?.watchFolder || $tw.boot.wikiTiddlersPath || './tiddlers');
  ...
  function toTWUTCString(date: Date): string {
    return `${date.getUTCFullYear()}${pad(date.getUTCMonth() + 1)}${pad(date.getUTCDate())}${pad(date.getUTCHours())}${pad(date.getUTCMinutes())}${pad(
      date.getUTCSeconds(),
    )}${(date.getUTCMilliseconds() / 1000).toFixed(3).slice(2, 5)}`;
  }

  const { files: initialLoadedFiles } = $tw.boot;

  const { inverseFilesIndex } = $tw.wiki.watchFs;
  ...
  for (const tiddlerTitle in initialLoadedFiles) {
    if ({}.hasOwnProperty.call(initialLoadedFiles, tiddlerTitle)) {
      ...
      inverseFilesIndex.set(fileRelativePath, { ...fileDescriptor, filepath: fileRelativePath, title: tiddlerTitle });
    }
  }
  ...
  const updateInverseIndex = (filePath, fileDescriptor) => {
    if (fileDescriptor) {
      $tw.wiki.watchFs.inverseFilesIndex.set(filePath, fileDescriptor);
    } else {
      $tw.wiki.watchFs.inverseFilesIndex.delete(filePath);
    }
  };
  const filePathExistsInIndex = (filePath: string) => !!inverseFilesIndex[filePath];
  const getTitleByPath = (fileRelativePath: string) => {
    const cachedFile = $tw.wiki.watchFs.inverseFilesIndex.get(fileRelativePath);
    if (cachedFile) {
      return cachedFile.title;
    }
      ...
      if (!filePathExistsInIndex(fileRelativePath)) {
        tiddlers.forEach((tiddler) => {
          ...
          if (existedWikiRecord && deepEqual(tiddler, existedWikiRecord.fields)) {
            ...
            if (fileDescriptor.tiddlerTitle && fileDescriptor.tiddlerTitle !== `${tiddler.title}.tid` && fileDescriptor.tiddlerTitle !== tiddler.title) {
            ...
            debugLog('get new addTiddler fileDescriptor.tiddlerTitle', fileDescriptor.tiddlerTitle, 'tiddler.title', tiddler.title);
            updateInverseIndex(fileRelativePath, { ...fileDescriptor, tiddlerTitle: tiddler.title });
    ...
    if (changeType === 'unlink') {
      ...
      if (!existedTiddlerResult) {
        ...
        fs.writeFile(fileAbsolutePath, '', {}, () => {
          ...
          try {
            if (fileAbsolutePath.startsWith('$') && fs.existsSync(fileAbsolutePath) && fs.readFileSync(fileAbsolutePath, 'utf-8').length === 0) {
              fs.unlinkSync(fileAbsolutePath);
            }
          } catch (error) {
            console.error(error);
